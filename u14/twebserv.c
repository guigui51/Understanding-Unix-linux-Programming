//通过对传输的头来解析请求几请求后续内容，根据不同的请求和后续来实现相关操作
...
main(int ac, char *av[])
{
	...
	sock=make_server_socket(atoi(av[1]));  //得到sockid，和绑定监听
	if(sock==-1){perror("making socket"); exit(2);}
	
	setup(&attr);	//初始化线程属性为分离态，时间请求发送数据数置0
	
	while(1){
		fd=accept(sock, NULL, NULL);	
		server_requests++;
		fdptr=malloc(sizeof(int));//fdptr=4;
		*fdptr=fd;
		pthread_create(&worker, &attr, handle_call, fdptr);		//创建线程，执行请求
	}
}
...
setup(pthread_attr_t *attrp)
{
	...
	pthread_attr_setdetachstate(attrp, PTHREAD_CREATE_DETACHED); //创建线程为分离态
...
void *handle_call(void *fdptr)
{
	...
	skip_rest_of_header(fpin);		//读完所有的头，并且省略
	
	process_rq(request, fd);  //fd就是sock
